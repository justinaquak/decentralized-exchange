...
Click on ```Request``` to request GOLD, SILVER and BRONZE ERC20 tokens

<img width="1246" alt="image" src="https://user-images.githubusercontent.com/73157214/198841535-aa7dc3ea-795b-42fb-ad6e-8a84574ba7bf.png">
...
Faucet requests are supposed to be rate limited to once per day for each account. However, for testing purposes we adjusted the rate limit for respective accounts to once every two minutes. 

![image](https://user-images.githubusercontent.com/72204360/198329511-175d4080-4984-44c8-9d05-103763249b84.png)

# Limit Orders

To place a buy/sell limit order, select the token you want to buy/sell on the left, with the price and the amount you would like. On the right, select the token you would like to exchange with. Once done, click on the exchange icon in the middle to execute the trade. Take note of the account selected at the top panel, as this is the account that would be executing the trade. If there are suitable orders, the order will be fulfilled instantly, else they will be added into the orderbook. 
...
After doing so, the orderbook should be updated, and the user's balance should be updated as well. This is because the exchange will lock-up the tokens. In the case of a buy order, the token on the right will be locked-up. In the case of a sell order, the token on the left would be locked-up. In the example shown above, it was a a buy order, and hence the BRONZE token was locked-up by the exchange. 

![image](https://user-images.githubusercontent.com/73157214/198842178-aab0e61c-1adb-4500-8d47-ab39e3963ec5.png)
...
This feature is to support a different method of order matching. For example, in the case where Alice wants to buy 1 GOLD token for 10
SILVER tokens, Bob wants to sell 10 SILVER tokens for 100 BRONZE tokens, and Charlie wants to sell 100 BRONZE tokens for 1 GOLD tokens, then batched execution will facilitate the token swaps among Alice, Bob and Charlie.
...
![image](https://user-images.githubusercontent.com/73157214/198842557-f2d32905-e8e7-480e-b60a-7bb82da7c033.png)
