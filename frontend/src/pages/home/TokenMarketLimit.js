...
function TokenMarketLimit(account, setAccountInfo, setData) {
  ...
  const buyTokenLimitMarket = (tokenA, tokenB, amount, user, value) => {
    axios
      .post(
        `${defaultAPI}orders/buyTokenLimit?tokenA=${tokenA}&tokenB=${tokenB}&tokenBAmount=${amount}&user=${user}&tokenBPrice=${value}`
      )
      .then((res) => {
  ...
  const sellTokenLimitMarket = (tokenA, tokenB, amount, user, value) => {
    axios
      .post(
        `${defaultAPI}orders/sellTokenLimit?tokenA=${tokenA}&tokenB=${tokenB}&tokenBAmount=${amount}&user=${user}&tokenBPrice=${value}`
      )
      .then((res) => {
        if (res.data.result === "Failed") {
          message.error(res.data.message);
